  ...
  try
    ...
    begin
      if not IsNumero( PChave ) then
        UmDM.QEstados.SQL.Text := 'select * from estados where estado like ' + Chr( 39 ) + '%' + PChave + '%' + Chr( 39 ) + ' Order by estado'
      else
...
begin
  ...
  try
    ...
    UmDM.QEstados.FieldByName( 'COD_PAIS' ).AsInteger := MEstado.GetoPais.GetCodigo;
